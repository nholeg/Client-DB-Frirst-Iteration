...
END name, tel,
CASE 
WHEN ROW_NUMBER() OVER (PARTITION BY inv ORDER BY typenum) =1 THEN inv
ELSE ''
END inv,
CASE 
...
ELSE ''
END remain
FROM ( SELECT DISTINCT c.cname as name, b.tel, i.inv , sum (p.paid) as sum_paid,
sum (i.inv) - sum (p.paid) as remain,
  CASE 
